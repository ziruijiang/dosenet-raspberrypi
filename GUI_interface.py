...
location.set(option3[0])
period = Tkinter.StringVar()
period.set(option4[0])
...
def make_run_gui():
    ...
    if varWeather.get():
        PressureButton = Tkinter.Button(top1, height=2, width=10, text = "Pressure Plot", command = press)
        PressureButton.grid(row=0, column=2)
        TempButton = Tkinter.Button(top1, height=2, width=10, text = "Temperature Plot", command = temp)
        TempButton.grid(row=0, column=3)
        HumidButton = Tkinter.Button(top1, height=2, width=10, text = "Humidity Plot", command = humid)
        HumidButton.grid(row=0, column=4)

    if varCO2.get():
        CO2Button = Tkinter.Button(top1, height=2, width=10, text = "CO2 Plot", command = CO2)
        CO2Button.grid(row=0, column=5)
        
    if varAir.get():
        AirButton = Tkinter.Button(top1, height=2, width=10, text = "Air Quality Plot", command = airquality)
...
AirButton = Tkinter.Checkbutton(top, text="Air Quality", variable=varAir, font="Times 20").grid(row=0, column = 3,pady = 2)    
WeatherButton = Tkinter.Checkbutton(top, text='Weather Sensor', variable=varWeather, font="Times 20").grid(row=1, column = 3,pady = 2)
CO2Button = Tkinter.Checkbutton(top, text="CO2 Sensor", variable=varCO2, font="Times 20").grid(row=2, column = 3,pady = 2)
d3sButton = Tkinter.Checkbutton(top, text="D3S", variable=vard3s, font="Times 20").grid(row=3, column = 3,pady = 2)
# maxdata = Entry(top)
# n_merge = Entry(top)
...
maxdata_text = Label(top, text = "Data points on plot", font = "Times 16").grid(row=4, column = 2,pady = 2, columnspan =2, sticky = Tkinter.E)
nmerge_text = Label(top, text = 'Integration seconds (s)', font = "Times 16").grid(row=5, column = 3,pady = 2,columnspan =2, sticky = Tkinter.E)
period_text = Label(top, text = 'Period', font = "Times 16").grid(row=6, column = 2,pady = 2,columnspan =2, sticky = Tkinter.E)
location_text = Label(top, text = 'Location', font = "Times 16").grid(row=7, column = 2,pady = 2, columnspan =2, sticky = Tkinter.E)
RecordButton = Tkinter.Button(top, text="Record Data", command = weather_test, font="Times 20").grid(row=8, column = 3)
 
maxdata_option.grid(row=4, column = 3,pady = 2, sticky = Tkinter.E) 
n_merge_option.grid(row=5, column = 3,pady = 2, sticky = Tkinter.E)
period_option.grid(row=6, column = 3,pady = 2, sticky = Tkinter.E) 
location_option.grid(row=7, column = 3,pady = 2, sticky = Tkinter.E)
